...
export async function addProduit(data: Omit<Produit, "idprod">) {
  const {
    itemname,
    prix,
    quantity,
  ...
  try {
    await sql`
    INSERT INTO produit (itemname,prix,quantity,stockalerte,type,date_creation,description)
    VALUES (${itemname},${prix},${quantity},${stockalerte},${type},${date_creation},${description});
  `;
  } catch (error) {
...
export async function addDetailCommande(
  ...
  const total = Number(quantity) * Number(prix_unitaire);
  try {
    await sql`
