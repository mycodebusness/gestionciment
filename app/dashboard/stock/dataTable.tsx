...
import { Stock as Payment } from "@/lib/types";
...
export const columns: ColumnDef<Payment>[] = [
  {
    ...
    header: "Id",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("idstock")}</div>
    ),
  },
  {
    accessorKey: "date",
  ...
  },
  {
    ...
    header: "Emplacement",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("emplacement")}</div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize">
        {new Date(
          row.original.date_expiration || Date.now()
      ...
      return (
        <div className="flex gap-3">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Link className="flex gap-3" href={`/dashboard/stock/ajouter`}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  ...
                  href={`/dashboard/stock/${row.original.idstock}`}
                >
              ...
              <TooltipContent className="flex gap-2 justify-center items-center">
                ...
                <p>modifier le stock N°{row.original.idstock}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <Dialog>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="flex gap-2 justify-center items-center">
                  ...
                  <p>supprimer le stock N°{row.original.idstock}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle> Suppression stock</DialogTitle>
                <DialogDescription>
                  Etes-vous sur de supprimer le stock {row.original.idstock}{" "}
                  définitivement?
                </DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter className="justify-end">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await deleteStock(row.original.idstock);
                      toast({
                        title: "Supprimer",
                        description: `le stock ${row.original.idstock} a été supprimer avec succès`,
    ...
    <div className="w-full px-4 pt-4">
      <div className="flex items-center pb-4">
        <Input
          ...
          value={(table.getColumn("idstock")?.getFilterValue() as string) ?? ""}
          onChange={(event) =>
            table.getColumn("idstock")?.setFilterValue(event.target.value)
          }
          className="max-w-sm"
        />
      </div>
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="flex-1 text-sm text-muted-foreground">
          Total : {table.getFilteredRowModel().rows.length} stock(s).
        </div>
