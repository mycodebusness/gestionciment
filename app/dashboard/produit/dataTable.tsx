...
import { Produit as Payment } from "@/lib/types";
...
import { deleteProduit } from "@/lib/actions";
...
export const columns: ColumnDef<Payment>[] = [
  {
    ...
    header: "Id",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("idprod")}</div>
    ),
  },
  {
  ...
  },
  {
    accessorKey: "prix",
    header: "Prix",
    cell: ({ row }) => <div className="capitalize">{row.getValue("prix")}</div>,
  },
  {
    accessorKey: "stockalerte",
    header: "Qte alerte",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("stockalerte")}</div>
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize">
        {new Date(row.original.date_creation || Date.now())?.toLocaleDateString(
          "fr-FR",
          {
            month: "short",
            day: "numeric",
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  ...
                  href={`/dashboard/produit/ajouter`}
                >
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  className="flex gap-3"
                  href={`/dashboard/produit/${row.original.idprod}`}
                >
              ...
              <TooltipContent className="flex gap-2 justify-center items-center">
                ...
                <p>modifier le produit {row.original.itemname}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <Dialog>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="flex gap-2 justify-center items-center">
                  ...
                  <p>supprimer le produit {row.original.itemname}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Etes-vous sur de supprimer le produit {row.original.itemname}{" "}
                  définitivement?
                </DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter className="justify-end">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await deleteProduit(row.original.idprod);
                      toast({
                        title: "Supprimer",
                        description: `le produit ${row.original.itemname} a été supprimer avec succès`,
    ...
    <div className="w-full px-4 pt-4">
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="flex-1 text-sm text-muted-foreground">
          Total : {table.getFilteredRowModel().rows.length} produit(s).
        </div>
